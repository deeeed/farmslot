...
- fix(run-engine): interactive dev runs stop at the human gate again. The gate was skipped for the lightweight profile even though `isHumanGateEnabled` already returns true for every interactive run, and self-review is skipped too, so a worker terminal signal ran straight through to completion — releasing the slot with uncommitted work, no branch commits and no PR. The gate is where the operator resolves the run.
