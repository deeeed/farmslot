...
async function writeChecklistMarker(taskAbsDir: string, farmslotDirForSlot: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      '  TASK="$DIR/CHECKLIST.md"',
      'fi',
      'if [ -f "$DIR/CI-FIX.md" ]; then',
      '  TASK="$DIR/CI-FIX.md"',
      '  SIGNAL="$DIR/CI-FIX-SIGNAL.json"',
      'elif [ -f "$DIR/SELF-REVIEW-FIX.md" ]; then',
      '  TASK="$DIR/SELF-REVIEW-FIX.md"',
      '  SIGNAL="$DIR/SELF-REVIEW-FIX-SIGNAL.json"',
      'elif [ -f "$DIR/SELF-REVIEW.md" ] && { [ ! -f "$DIR/TASK.md" ] || [ "$DIR/SELF-REVIEW.md" -nt "$DIR/TASK.md" ]; }; then',
      '  TASK="$DIR/SELF-REVIEW.md"',
      '  SIGNAL="$DIR/SELF-REVIEW-SIGNAL.json"',
      'fi',
      `node ${JSON.stringify(helperPath)} "$TASK" "$SIGNAL" "$@"`,
      '',
    ].join('\n'),
    'utf-8',
  );
