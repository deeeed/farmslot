...
describe('PR list cache', { concurrency: 1 }, () => {
  ...
  test('the refresher only fetches while a client is connected and recently asked for pr.list', async () => {
    ...
    try {
      ...
      assert.ok(
        calls >= 2,
        'a connected client with recent pr.list interest refreshes a stale copy',
      );
  ...
  test('a stale warm list is not background-refreshed while GraphQL is reserved', async () => {
    const { cleanup } = isolate();
    try {
      let calls = 0;
      const fetch = async () => {
      ...
      await servePRList(fetch, { now: t0 });
      assert.equal(calls, 1);
      githubQueryBudget.observe('hashed', {
        remaining: 0,
        cost: 1,
      ...
      const served = await servePRList(fetch, { now: Date.now() });
      await new Promise((resolve) => setTimeout(resolve, 0));
      assert.equal(calls, 1, 'reserved budget must not start a background fetch');
      assert.equal(served.prs[0].pr, 1);
      assert.equal(served.refreshing, false);
      assert.equal(events.length, 0, 'no refresh-failed broadcast');
    } finally {
      cleanup();
    }
  });
  ...
  test('the carry clock survives a gateway restart', async () => {
    ...
    try {
      ...
      const stored = JSON.parse(
        readFileSync(path.join(dir, '.farm-cache', 'pr-list.json'), 'utf-8'),
      );
