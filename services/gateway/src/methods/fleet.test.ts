...
test('preserveClaimedRowsUnknownToProbe keeps live claims a stale refresh never probed', () => {
  const rows = [
    ...
    // Ownership markers are required: a busy row with neither owner nor
    // reservation is a ghost and must not be preserved forever.
    { slot: 'ownerless-busy', lifecycle: 'busy', current_run_id: null },
  ];
