  ...
  async start(options, emit) {
    ...
    const finishTurn = (data: JsonRecord = {}) => {
      ...
      publish({
        type: 'turn.completed',
        status:
          interruptedTurnId !== undefined && interruptedTurnId === turnId
            ? 'interrupted'
            : data.isError === true
              ? 'failed'
      ...
      if (message.type === 'control_response') {
        ...
        if (pendingInterrupt && response.request_id === pendingInterrupt.requestId) {
          ...
          if (response.subtype === 'success') {
            ...
            if (turnId === pending.turnId) interruptedTurnId = pending.turnId;
            pending.resolve();
          } else {
    ...
    process = new JsonLineProcess(
      ...
      (error) => {
        ...
        publish({
          type: 'session.closed',
          ...(failure ? { status: 'failed', data: { error: failure.message } } : {}),
        });
      },
    );
    ...
    return {
      ...
      async interrupt() {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          pendingInterrupt = { requestId, turnId: interruptedId, resolve, reject, timer };
          try {
            process.write({
              type: 'control_request',
              request_id: requestId,
