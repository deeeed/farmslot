...
describe('worker-prompt', () => {
  it('resolveWorkerDispatchPrompt expands TASK_FILE from farmslot template', async () => {
    ...
    assert.match(prompt, /Read temp\/tasks\/feat\/tat-3215-0626-211118\/TASK\.md first/);
  ...
  it('resolveWorkerNudgePrompt expands TASK_FILE from farmslot template', async () => {
    ...
    assert.match(
      prompt,
      /New task waiting at \/Users\/dev\/repo\/temp\/tasks\/feat\/demo\/TASK\.md/,
    );
    assert.match(prompt, /\/Users\/dev\/repo\/temp\/tasks\/feat\/demo\/mark N/);
  });

  it('falls back to farmslot when project template is missing', async () => {
    const prompt = await resolveWorkerDispatchPrompt('nonexistent-farm-project', {
      taskFile: 'temp/tasks/demo/TASK.md',
    });
    assert.match(prompt, /Read temp\/tasks\/demo\/TASK\.md first/);
  });
});
