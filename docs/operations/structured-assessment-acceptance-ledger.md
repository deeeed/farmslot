...
| Textual acceptance criteria / evidence | Opt-in advisory text-only assessment shipped in #730; visual/mixed proof refused; authoritative ledger unchanged | v2 Jev adapter probe: 8/9 held-out with a wrong definite verdict; v3 gateway Jev probe: 9/9 held-out, 11/12 overall, with a wrong definite verdict on a development case | None                                                                                                                                                                                | Keep routine Jev calls off; the small v3 classification pass and development error need paired validation before any benefit claim         |
...
On September 24, 2026, a separate [operator read](../../scripts/acceptance-evidence/results/v3-independent-label-read.json)
agreed with all 12 frozen text labels and both no-call decisions. The dated
...
assessment records, and two visual/mixed refusals before transport. Jev matched
...
with recorded confidence 0.99. The gateway recorded 6,132 input and 561 output
...
0.01. The price snapshot was checked against
[TypeSafe's model page](https://docs.typesafe.ai/models) on the run date.
