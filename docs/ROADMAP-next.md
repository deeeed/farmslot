...
- **Run metrics surfacing follow-ups — producer tooling and per-model cost** — PR #177 shipped the root duration helper and single recipe-quality signal: blocked/gated runs now get real elapsed time and leaderboard/per-run recipe badges consume `loadRecipeQualityEvaluation.signal`. Remaining work is producer/tooling and per-model breakdown, not the original divergent-reader bug.
  - **Remaining root cause — recipe-quality producer/tooling gap.** The canonical `RecipeQualityArtifact` schema lives only as a TS type guard (`services/gateway/src/quality/recipe-quality.ts` `isRecipeQualityArtifact`). The worker spec (`projects/farmslot-farm/fixtures/runtime/recipe-quality.md`) tells the agent to _write_ `recipe-quality.json` but gives **no schema, fields, or example**, and the worker-side validator (`scripts/quality/check-task-artifact-contract.mjs`) only checks a loose 3-field subset (`version`, `verdict`, `compact.verdict`). So agents infer a non-conformant shape that passes the loose check but fails the strict gateway guard → `loadRecipeQualityEvaluation` fabricates a `verdict:'fail'` artifact → leaderboard `bad`. Unlike `recipe.json` (which has `farmslot recipe validate` + `recipe-doctor.mjs`), `recipe-quality.json` has no agent-facing validator or generator.
  - **Fix — one schema, real tooling, one signal.** (1) Define the `RecipeQualityArtifact` schema once (protocol, with an exported JSON schema or shared validator) and make `check-task-artifact-contract.mjs` validate the **full** schema via the same guard the gateway uses — so a wrong file fails the worker's own gate, not silently downstream. (2) Provide a generator/builder tool (wraps the gateway's `evaluateRecipeQuality`) so the agent supplies only verdict + reasons and the tool emits the conformant structure — eliminating inference cost. (3) Document the schema + a complete example in `recipe-quality.md`. (4) Keep `loadRecipeQualityEvaluation.signal` as the single recipe signal shipped in PR #177 while adding producer-side schema validation/tooling around it.
