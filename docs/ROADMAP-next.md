...
**Last updated:** 2026-06-30 (through PR #190: release maturity diagnostics, WorkGraph/roadmap promotion, Codex/scripted-runner reliability, installed capture-helper, package release)
**Stale by:** 2026-08-22
...
- **Run metrics surfacing follow-ups — agent-runtime extraction and per-model cost** — PR #177 shipped the root duration helper and single recipe-quality signal: blocked/gated runs now get real elapsed time and leaderboard/per-run recipe badges consume `loadRecipeQualityEvaluation.signal`. Remaining work is contract alignment, reusable task runtime packaging, producer tooling, and per-model breakdown.
  - **Contract alignment / agent-runtime lane.** `RecipeQualityArtifact` should be validated by `@farmslot/protocol` and enforced by `@farmslot/agent-runtime` task checks, with `@farmslot/skills` reduced to instructions plus compatibility shims. `recipe-quality.json` producer tooling still needs a generator/builder so agents provide verdict + reasons and receive a conformant structure instead of hand-authoring the full schema.
