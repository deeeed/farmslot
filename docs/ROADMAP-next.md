...
**Last updated:** 2026-06-30 (consolidate run-metrics surfacing: single-source recipe quality/duration/per-model + recipe-quality producer tooling)
**Stale by:** 2026-08-22
...
2. **Run a short operator UI/UX stabilization pass.** Exercise the recently changed Command Center surfaces (`#evals`, `#backlog`, dispatch, run detail/family observability, slot view, ready/review workspaces, and dev publication gate) plus the newly shipped Mobile Companion operator surfaces (active runs, artifacts/diff/recipe workspaces, PRs, terminal, and Workers). Fix concrete regressions and polish issues found during real operator use. This is stabilization of shipped roadmap work, not a new product lane; small run-history curation affordances such as operator tags for demo/review collections belong here. Companion structural refactor (screen controllers, feature folders, shared workspace kit) is captured in [plans/companion-ui-architecture-refactor.md](plans/companion-ui-architecture-refactor.md). Add feature-maturity `ALPHA` labels to under-tested Command Center surfaces (Roadmap, Backlog, Analytics, Finetune, Intelligence, and Graphs) until each has CDP smoke coverage, validated empty/error states, and real operator-use evidence. Include release/professionalization UX in this stabilization lane: version-footer details for UI/gateway/node/protocol identity, clearer stable/preview/dev build-channel status, and a progressive Doctor experience that can refresh individual sections without freezing the whole page. Stable release-channel update behavior and unified release scripting are follow-up release-engineering work after this UI/proof slice.
...
- **Work-graph orchestration + roadmap promotion** shipped across PR #157 (`93652cf`, compose work graphs from promoted specs) plus the squash-merged work-graph cluster (`e15396b` idea refinement + backlog promotion, `3c76f96` backlog dependency orchestration, `4210145` cross-project backlog dependency visualization, `f2f4c7b` execution overlay + dispatch config), 2026-06-29 — promoted specs become work graphs with cross-project backlog dependencies, an execution overlay, and dispatch config.
...
- **Global capture-helper resolution** — _Captured 2026-06-30 from PR #180 validation._ Operator and isolated worktree Doctor runs can disagree because `capture-helper` resolution still depends on each gateway process PATH/wrapper environment. Define the intended single global capture-helper install/resolution contract, migrate Doctor/recipe checks to that contract, and only then make Command Center diagnostics report the resolved binary/version consistently.
...
- **Consolidate run metrics surfacing — single source for recipe quality, duration, and per-model cost** — _Approved 2026-06-30; consolidation lane._ The family comparison surfaces (Leaderboard/Matrix) showed inconsistent data for the same run (`recipe = bad` on the leaderboard while the per-run panel said `pass`; `duration = —`; no per-model breakdown) because the same metric is computed from several diverging sources. This lane makes each metric single-source and tools the producer so agents stop inferring formats.
  - **Root cause — recipe-quality dual-schema drift + no producer tooling.** The canonical `RecipeQualityArtifact` schema lives only as a TS type guard (`services/gateway/src/quality/recipe-quality.ts` `isRecipeQualityArtifact`). The worker spec (`projects/farmslot-farm/fixtures/runtime/recipe-quality.md`) tells the agent to _write_ `recipe-quality.json` but gives **no schema, fields, or example**, and the worker-side validator (`scripts/quality/check-task-artifact-contract.mjs`) only checks a loose 3-field subset (`version`, `verdict`, `compact.verdict`). So agents infer a non-conformant shape that passes the loose check but fails the strict gateway guard → `loadRecipeQualityEvaluation` fabricates a `verdict:'fail'` artifact → leaderboard `bad`. Unlike `recipe.json` (which has `farmslot recipe validate` + `recipe-doctor.mjs`), `recipe-quality.json` has no agent-facing validator or generator.
  - **Fix — one schema, real tooling, one signal.** (1) Define the `RecipeQualityArtifact` schema once (protocol, with an exported JSON schema or shared validator) and make `check-task-artifact-contract.mjs` validate the **full** schema via the same guard the gateway uses — so a wrong file fails the worker's own gate, not silently downstream. (2) Provide a generator/builder tool (wraps the gateway's `evaluateRecipeQuality`) so the agent supplies only verdict + reasons and the tool emits the conformant structure — eliminating inference cost. (3) Document the schema + a complete example in `recipe-quality.md`. (4) Make `loadRecipeQualityEvaluation.signal` the single recipe signal consumed by both the leaderboard and the per-run badge (the PR #175 lenient badge reader was reverted in favor of this — keep only the single-source token fix from #175).
  - **Duration single-source.** Promote one `runDurationMs(run)` helper (timestamps; blocked = `createdAt`→gate-entry) used by `family-observability/snapshot.ts`, the leaderboard, and `finetune.ts` (currently 3–4 separate computations; snapshot reads raw `metrics.durationMs` = null for blocked runs).
  ...
  - **Verification:** leaderboard + per-run badge + matrix agree for a run; duration populated for blocked-at-gate runs; `metrics.models[]` shows worker vs self-review vs sub-agent split; a non-conformant `recipe-quality.json` fails `check-task-artifact-contract.mjs` locally. CDP-validate the family Leaderboard + Matrix.
