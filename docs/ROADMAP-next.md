...
7. **Ship runner observability via runner hooks (ADR-032 phase 1).** Execute [plans/runner-observability-hooks-phase1.md](plans/runner-observability-hooks-phase1.md): add telemetry-only Farmslot hook/statusline writers that emit `hooks.jsonl` + `statusline.json`, then compare hook-derived state with pane-derived state before making hooks authoritative. Motivated by the 2026-05-21 mm-3 regression (`· Composing…` spinner not in regex set, coincided with the operator's first Claude version upgrade). **Shipped in PR #81 closeout:** Claude + Codex hook installers, gateway `event-driven` Codex scope, and the **`scripts/runner-validation/` harness** — per-runner launch adapters plus tmux scenarios for hook smoke, prompt-accepted digest correlation, turn boundaries, busy-composer pane fixtures, permission-mode checks, **pane-smoke** (Cursor/Grok), and **interaction-smoke** (Grok production TUI path). Operator guide: [operations/runner-validation-harness.md](operations/runner-validation-harness.md). Run via `node scripts/runner-validation/run.mjs --runner all`; evidence lands under `docs/operations/evidence/runner-validate-*`. Grok is first-class (`pane-only` + interactive parity); extend gate to `pane-only` when Grok slots become fleet-default. Remaining before hooks become authoritative: fleet `$TMUX_PANE` verification on runner-local + mini + runner-a, 200-event agreement window, plan-mode live check, Cursor/Grok stay `pane-only` with explicit skip reasons in the harness. Codex production launches still need `CODEX_HOME` wired through `launch-command.ts` (installer/bootstrap lands in this PR; gateway prefix is follow-up if not merged here).
