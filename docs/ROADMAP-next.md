...
7. **Monitor ADR-032 Phase 2 exit and schedule Phase 3 after the next Claude minor upgrade.** Phases 1–2 are shipped (PRs #81–#84 plus post-launch obs-first follow-up through `91674d9`): hook installers, `scripts/runner-validation/` harness, `sendRunnerInstructionSafely` obs-first path, and `resolveSafeSendTimeoutMs()`. Remaining work is empirical only: `Run.metrics.nudgeTimeoutCount` zero over a 7-day window on Claude slots, optional agreement-log triage, then Phase 3 retirement of Claude-only pane-regex branches. Do not reschedule Phase 1/2 implementation.
