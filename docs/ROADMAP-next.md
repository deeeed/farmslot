...
- **Gate-held worker session (publication + review gates)** — operator expectation from 2026-06 TAT-3398: at the human publication gate the worker should still be attachable in `dev` (or primary role window) so the reviewer can ask why/how questions with full task context. **Today this is not true for `dev` or `fix-bug`:** both flows share the same pipeline (`… → COMPLETE → HUMAN_GATE`) and `shouldPrepareLocalFirstPackage` calls `slotRelease` inside COMPLETE, which runs `killAllAgentWindows` + `killAgentInSession` before the gate opens; the shipped test explicitly asserts human-gate can approve after the slot was released. `review-pr` is the only gate-first flow (`… → HUMAN_GATE → COMPLETE`), so its worker is more likely to still be in tmux when the operator reviews — that ordering is the model to generalize, not a separate bugfix-only mechanism. Secondary loss path: dispatch sets `pane-died → kill-pane` on role windows, and worker templates (`dev.md`, `fix-bug.md`) do not instruct "stay alive / do not `/exit`" (unlike `ci-fix.md`). **Proposed fix (re-plan before coding):** (1) split local-first COMPLETE into package-prep without agent teardown — defer `slotRelease` agent kill until FINALIZE (or gate resolution) while keeping `keepWork` + `detachRuns: false`; (2) mark slot `review-gate` during the held period (HUMAN_GATE already does this when `slotId` is still assigned); (3) add template tail guidance on `dev.md` / `fix-bug.md` to remain idle after `SIGNAL.json` until gateway publication; (4) gate Companion/tmux "talk to worker" affordances on `agentContexts[].status` + live runner-under-pane check. Session-resume on relaunch remains the fallback when the runner exits anyway. Aligns with [PRD-runner-execution-canonical.md](PRD-runner-execution-canonical.md) §2 and local-first publication gates in PR #73 / PR #96.
