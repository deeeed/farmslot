...
- **Experimental worker session history panel ([ADR-047](adr/047-worker-session-history-panel.md))** — _Captured 2026-07-02 from feasibility spike._ Read-only, chat-shaped **history browser** for active worker sessions: project runner-owned JSONL/session files (`runnerSessionPath`) instead of tmux scrollback or gateway LLM API calls — preserves subscription-backed runners. Primary value is **cleaner turn/tool history review**, not a second chat client. v1: slot-view **Terminal | History** tab (experimental label), transcript-backed Claude/Codex/Grok; Cursor TUI degraded. Input via `terminal.send` is explicitly deferred. Kill criteria in ADR if fidelity or adoption is poor. **Do not merge implementation until ADR-047 is accepted.**
