...
- **Project.** `TaskStepProgress` gains:
  ...
  `taskProgress` builds the child schema with `generateTaskSchema(childMarkdown, run.flowType)`; the flow type only labels the schema, enumeration is flow-independent. `'stale'` is projected when the child file says `running` and no mark event landed within `SUBTASK_STALE_AFTER_MS`, 15 minutes, exported from the gateway. A child unit has no runner of its own, so its only liveness evidence is its mark timestamps; worker idleness is decided by the monitor from runner-native probes and nudge budgets and is not a millisecond value any module exports. Depth is capped at 1 in v1 by the mark engine, not the schema.
...
| 2     | gateway watch + projection + acceptance rule + copy/mirror + terminal check; task-directory contract rows for `subtasks/` (layout, travel, mirror, layers) | `cdp.mjs gateway task.progress` shows `subtask` under the step on a live slot, including a child registered on `SELF-REVIEW.md` during an active self-review round; `subtasks/<name>.worker` files exist after completion; stale projection after `SUBTASK_STALE_AFTER_MS`. The scripted-runner scenario `yarn e2e:subtask-gateway` covers this phase and is a LOCAL pre-merge gate run from the operator checkout, not a CI job: it needs tmux, a pool slot, and a project directory, none of which exist on a CI runner (the Phase 1 scenario needs none of those and does run in CI, inside the `@farmslot/agent-runtime` package tests). |
