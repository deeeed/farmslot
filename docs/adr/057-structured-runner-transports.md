...
Worker dispatch must reserve and persist a lowercase session UUID before launch, then call `native.session.ensure`. Retrying that operation requires the same owner and launch configuration and returns the existing session, including closed or failed state. It never relaunches a terminal reservation. Ordinary `native.session.create` still creates a new session or explicitly resumes a saved conversation. Older nodes reject the distinct ensure operation; a newer client also refuses ensure against a retained host without that capability while preserving its ordinary session controls.
