...
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| Is the worker idle (ready to accept tmux send-keys)? | Hook JSONL emit on `Stop` / `SubagentStop` with no later `UserPromptSubmit`                                                                                         | `paneShowsBusyComposer`           |
...
export type RunnerActivity =
  | 'idle' // last event was Stop/SubagentStop
...
| `codex`    | Hooks emitted at `.codex/hooks.json` registration; OMX wrappers consume them today but only write workflow-mode state                           | OMX writes `.omx/state/<mode>-state.json` — workflow mode, not turn boundaries                                                                  | Not a TUI — no busy/idle markers exist                                                            | **Phase 1.5** — Farmslot-owned wrapper alongside OMX | Codex CLI exposes the hook surface. Farmslot writes its own wrapper script (parallel to OMX's, not replacing) that emits turn-boundary events to `hooks.jsonl`. Prerequisite: verify `.codex/hooks.json` accepts multiple wrapper registrations without conflict.                                               |
