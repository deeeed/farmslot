...
**Bounded process attribution (2026-08-20):** The private node-to-gateway metrics sample may attach one ownership-safe process census (`pid`, `ppid`, CPU, RSS, elapsed time, executable name/path; never argv or environment). It reuses the metrics cadence, runs at most once per minute under pressure and once per five minutes otherwise, and is capped by time, output bytes, and retained entries. Concurrent requests share one census while merging newly observed ownership PIDs into that sample. The gateway stores the census separately from `MachineHealth.system`, so fleet health events and ordinary health readers never receive raw PID/path data. The admin-only pressure RPC caps machines, history, groups, and cleanup candidates, reports omitted counts, scopes managed identities to project filters, and reuses resolved resource definitions for 30 seconds. Command Center fetches the whole-machine snapshot once and applies global selectors locally to cards and cleanup rows, avoiding a new gateway join on every filter change. The Co-Pilot projection removes PIDs, paths, and exact cleanup targets and caps history/groups again. Attribution is display-only: stale/manual/unknown classes do not authorize process termination, and cleanup remains exclusively hook-driven through `resource.cleanup`.
