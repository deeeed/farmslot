...
| [060](../adr/060-sub-task-observability.md)                | Sub-task observability             | Accepted   | Shipped        | Phase 4 live proof on a real run; per-sub-agent cost roll-up                 |
...
| Phase 3: Claude pane branches in shared monitors    | Partial | Run-monitor **stuck** uses `RunnerObservability.getActivity` / `getTurnState` and fails closed when activity is unknown (Cursor pane-only). `runnerLineLooksWaiting` remains for run-monitor waiting plus self-review/ci-monitor completion/relaunch — those are not yet on hooks |
| Cursor/Grok hook path                               | Partial | Cursor remains `pane-only`; Grok uses its native session event/history protocol for activity and exact prompt acceptance, with the generic pane fallback only while native activity is unknown                                                                                    |
...
| Canonical graph + composition (`workflow`, `call`, `paramsSchema`, `proofTargets`, teardown) | Shipped | Protocol tests + `@farmslot/recipe-harness`                                                                                           |
...
| `prepare.core` + explicit compatibility profile          | Shipped | Core is implicit when declared; named legacy profiles and the compatibility alias remain explicit                                                                                                                                           |
| Lazy proof-capability catalog and durable leases         | Shipped | Shared protocol, Gateway registry/store, task context, Farmslot providers, and Command Center projection                                                                                                                                    |
...
**Implementation: Shipped (PRs #681–#685, released in #687); live end-to-end proof and cost roll-up remain**
...
| Recursive progress projection, watched and broadcast     | Shipped       | gateway `subtasks/index.json` watch and `TaskStepProgress.subtask` projection (#682)                |
...
**Follow-up:** one real dev run and one static review run on a slot with the child visible end to end (the Phase 4 live proof); per-sub-agent cost roll-up when the per-model capture lane lands.
