...
| `runtime.capability.stopWarm` | rpc-only |             |     | Interim: no dedicated typed subcommand yet; use `farmslot rpc <method> [json]`. ADR-054 slice 3 promotes this to `farmslot resource` alongside the posture commands. Also used by the Command Center Slot View to stop a warm provider. |
