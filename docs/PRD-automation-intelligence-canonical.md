...
Review policy includes the existing Continue/Fresh session choice. Review rounds are static; runtime validation is a separate QA request with a farm preset ([ADR-058](adr/058-static-review-and-farm-owned-qa.md)). Default follow-up rounds to Continue for the same PR and review profile. Retain the selected reviewer identity, native session reference, reviewed SHA and findings so an incremental follow-up supplies the new diff and unresolved findings to that reviewer. Initial rounds start fresh. A requested full independent review starts fresh under the existing review-scope contract.
...
| TR-17 | Follow-up review rounds honor configurable Continue/Fresh choices and stay static; runtime validation uses QA. Continue resumes the compatible native reviewer session with prior findings and the changed diff; unavailable sessions show an explicit fresh fallback. Restart preserves the review chain, no slot is reserved between rounds, and each verdict records the SHA actually reviewed. |
