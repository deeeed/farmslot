...
| Gateway protocol      | WebSocket frames, method registry, `protocol.capabilities`, CLI access, generated docs, and public API reference pages exist.                         | Curate the most important gateway methods with stronger examples and tool/client guidance.                          | LLM/tool clients can safely discover and operate capabilities through policy.                                                          |
...
4. **Publish or defer remaining packages deliberately**: `@farmslot/protocol` and `@farmslot/recipe-harness` are public; `@farmslot/skills`, `@farmslot/expo-recipe`, `@farmslot/cli`, and `@farmslot/theme` should not be claimed as published until they are actually released.
