...
| `artifact_available` | The project `artifact_check` hook exits 0 — a fast, seconds-scale probe reporting whether the profile's prebuilt artifact could be resolved (never a download or install). |
