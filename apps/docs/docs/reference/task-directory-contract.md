...
Both surfaces produce the same task directory. Farmslot adds gateway-owned files beside it and never renames a shared one. Three layers:
...
| Control plane             | the gateway, after or beside the worker | `diff.txt`, `diff-stat.json`, `iteration-diff*`, `self-review-N.*`, `independent-review-N.*`, `review-loop-N/`, `review-feedback.rev-*`, `review-result.rev-*`, `pr-package.{json,md}`, `publication-gate-*.md`, `session-metrics.json`, `workflow.mmd`; under `inputs/`: `template-provenance.json`, `runtime-capability-catalog.json`, planning and inherited context | Farmslot only                                                                                              |

The control-plane list is the one `isGatewayOwnedArtifactMirrorEntry` in `services/gateway/src/core/artifact-copy-policy.ts` recognises: those files are never mirrored back from the slot and never count as worker evidence. A new gateway feature adds a file to that list; it does not reuse a worker name.
