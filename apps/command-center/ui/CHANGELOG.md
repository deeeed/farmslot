...
- feat(posture): Command Center surfaces the ADR-054 run resource posture. Run Detail shows the effective posture beside the pipeline with its policy source, retained/warm/stopped/failed counts, the last transition outcome and time, and one row per capability giving the Gateway's desired disposition against the observed provider state — a released lease still inside its keep-warm window reads as running, not stopped. Human gates offer the four operator choices with the Gateway's effect preview before resolution and pass the typed `resourcePosture` param, surfacing a refusal such as park-ineligible instead of resolving the gate; the choices appear only where the Gateway would honour them, and a failed preview request is distinguished from a Gateway rejection. Backlog dispatch settings expose `waitPolicy`. Slot View separates lease state from provider state using the protocol's shared derivation rather than deciding locally, and offers acquire, restart, and stop recovery actions that reuse the capability's recorded proof requirement and parameters instead of inventing them. Restart confirms the Gateway actually released the lease before it reacquires, so it cannot start a second provider on top of one that never stopped; a warm provider is stopped through `runtime.capability.stopWarm` rather than the release RPC that skips it, reporting the Gateway's outcome as-is — a deferral names what still depends on the provider and keeps the control, a failed cleanup shows the failure, and a stop is only ever claimed when the Gateway observed one — and a failed action stays on screen instead of being cleared by the next refresh. Posture counts describe what providers are observed doing, so a provider that is still running or whose cleanup failed is never counted as stopped; the count, desired-versus-observed, and transition-failure de-duplication rules come from the shared protocol derivations rather than a Command Center copy. Choosing `project-default` at a gate no longer warns that the Gateway ignored the choice: deferring to the dispatch config, project, or framework defaults is exactly what that choice asks for, and the warning stays for a choice the Gateway really did not resolve from, including one ignored because the run left the operator wait before the preview ran. A refused plan is summarised as `nothing applied` rather than `no capability changes`, a held choice is dropped once the Gateway moves the run out of `operator-wait`, and a refusal only blocks Resolve while the choices are actually on screen, so a refused choice can never deadlock the decision from behind a hidden panel — including while the posture is unreadable, where the selection is kept rather than discarded, and the apply outcome is adopted only once the Gateway has recorded a transition for that resolution instead of the one it returned before reconciliation finished, with the correlation rules now taken from the shared protocol contract rather than a Command Center copy. A gate choice is forwarded only where the Gateway would honour it, so a refused choice can no longer be sent after a failed posture read hides the panel, and a withheld choice says so instead of being dropped silently. The resolution outcome and the reconciliation-pending notice moved to the persistent posture summary, which outlives the gate that disappears once the decision resolves, and a withheld choice is now reported there too rather than dropped in silence. Those notices render in every summary branch, including a failed posture read, which is exactly when a resolution is most likely still pending. The correlation baseline is built from the choice that was actually forwarded, so a withheld selection no longer makes the client wait forever for a record the Gateway attributed to something else, and a result matched positionally rather than attributed by the Gateway is described as the run's posture rather than as the operator's own outcome.
