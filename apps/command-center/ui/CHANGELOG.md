...
- feat(posture): the Run Detail posture rows show the device each capability lease actually resolved to, and the recipe replay controls take a target device — a key and a free-text identity, validated client-side against the protocol's identity charset — so a replay can run on another simulator or serial without re-dispatching the run. Leaving the field empty replays on the slot's configured device. The control offers the four device-identity keys only. `platform`, which the RPC also accepts to choose between an iOS and an Android provider, is not offered: every slot in the fleet is single-platform today, so a platform switch has no second provider to reach. Use `recipe.rerun` directly for that half.
