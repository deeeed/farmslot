...
- A checklist step that owns a child unit shows it nested under the step: source, status (running, blocked, stale, complete), its own step count, current step, and its steps on expand. Progress tracker, run-detail pipeline panel and step inspector all render it, and the parent's own counts are unchanged.
- Checklist timing in the gate summary (retrospective and publication gate) lists each child unit's own steps under the step it hangs off; a child whose parent step never got ticked still gets a row.
...
- Dispatch and comparison pickers include the `pi` runner, defaulting to `grok-4.6`, with PI `--thinking` levels for every PI model (default `medium`) and a hint for Ollama/LiteLLM/router model ids.
...
- chore(dev): machine pause dev fixtures carry the new `slotDisposition` field on preview runs, and the restore preview fixtures carry the `restoreTarget` the Gateway now serves — the slot a park would be restored into and whether it can take the run back.
...
- feat(posture): a run parked with `free-slot` says so wherever it is shown. Run Detail's posture panel names the parked state, the slot handed back to dispatch, the preserved branch and tip, and the slot a restore would use; the gate says that answering restores the run into that slot first, or shows the typed refusal (`RESTORE_SLOT_TAKEN` and friends) standing against it; and the run's Slot badge marks a freed slot as freed for dispatch instead of implying the run is sitting in it. The park notice renders even though a parked run hides the posture choices, which is exactly when the operator needs it. The machine pause dialog's restore rows carry the same summary with the Gateway's live availability verdict. Availability is stated only where the Gateway answered it; elsewhere it reads as not read. The freed slot is described as released by this run rather than free now, because a successor takes it routinely and the old wording sat on screen beside `RESTORE_SLOT_TAKEN`. A park that failed partway says which of the two it is — answerable where it stands, or owing a restore that waiting will not produce — and the Slot badge drops its restore promise once the run is terminal.
...
- feat(posture): Slot View separates lease state from provider state and offers acquire, restart, and stop recovery actions built from the capability's recorded proof requirement. Restart confirms the release before reacquiring, a warm provider is stopped through `runtime.capability.stopWarm`, and a failed action stays on screen.
- feat(dispatch): backlog dispatch settings expose `waitPolicy`.
- feat(posture): every posture surface reports what the Gateway observed, never what was intended: a released lease inside its keep-warm window reads as running, a provider is counted stopped only when a stop was observed, and a result the Gateway did not attribute to the operator's choice is described as the run's posture rather than as their outcome. The counts, the desired-versus-observed comparison, and the post-resolution correlation all come from the shared protocol contract.
