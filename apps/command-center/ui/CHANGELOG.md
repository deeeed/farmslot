...
- fix(runs): Complete Manually is available on failed runs and can prompt for an optional PR number. A failed slot release after the run is already `done` is shown in an alert. Replay is hidden once the hatch flag is set.
