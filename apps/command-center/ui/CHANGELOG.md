...
- Diff file lists (review and ready workspaces, slot workspace committed changes, diff modal) show the code/test split and a **Hide tests** toggle, remembered across views, so a quick PR review can focus on app code. Test files come from built-in patterns plus the project's `diff_view` overrides.
