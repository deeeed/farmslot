...
- Task progress now reaches a static review workspace run, which has no slot: run detail, the family and decision workspaces and the terminal screen all fetch it on open and accept its published updates (both keyed on the run, with the empty slot id such a run publishes under), so its checklist, child units and acceptance panel fill in immediately and refresh as the reviewer works instead of staying blank.
