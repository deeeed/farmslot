...
- feat(companion): harden mobile UX catalog — full route coverage (decision + worker terminal when context IDs set), offline HTML report cards, larger default-path primary touch targets, and Advanced backlog create (title + notes → `backlog.create`).
