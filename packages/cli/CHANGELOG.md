...
- feat: `farmslot fleet find-slot --project <name>` / `--slot <id>` — TypeScript port of the `scripts/find-slot.sh` decision core (selection scoring + per-slot blocker reasons), envelope-emitting with discriminated teaching errors (`NO_SLOT_AVAILABLE`, `SLOT_NOT_FOUND`, `SLOT_UNAVAILABLE`, `PROJECT_NOT_FOUND`).
