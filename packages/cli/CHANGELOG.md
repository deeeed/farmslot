...
- fix(dispatch): `farmslot dispatch pressure-admission status` reports the EFFECTIVE state. With
  ...
  setting alongside an override that supersedes it, and only claims dispatches are ungated when they
  are. Sustained-pressure dispatch prevention is now opt-in, so `enable` is an opt-in rather than a
  re-enable and the command copy says so.
- feat(resources): `farmslot resource capability status <slotId>` lists a slot's live leases and the fleet-wide queue behind the scoped claims they wait on, with each waiter's position, run, slot, and the run holding the claim. `resource posture status` names the claim a run is queued behind, and says when a claim is granted but its provider has not started, so a reservation nothing is completing is legible. A queued `capability acquire` reports the place in line it took rather than only that it was refused.
...
- feat(machine): add machine pause and restore previews, review-pinned execution with exact targets, durable status, selection controls, and partial-failure envelopes. `machine status` lists gate-parked runs — the slot disposition, the freed slot, the restore target, and the typed refusal from the last attempt on one line, with the preserved branch and tip beneath it — and `--json` carries the same list at `data.gateParks` beside the raw records. `machine status` probes nothing, so it reports the restore target without claiming the slot is free; `machine restore` is what asks. `machine restore` previews now print the same list with the Gateway's real availability verdict attached, and the header counts how many parks are actually holding a freed slot rather than how many are listed.
