...
function testLaneResultFallbacks() {
  const dir = mkdtempSync(path.join(os.tmpdir(), 'recipe-cook-validator-lane-'));
  writeJson(path.join(dir, 'recipe.json'), { title: 'x' });
  writeJson(path.join(dir, 'recipe-cook-learning.json'), {
    run_id: 'lane-run',
    scenario_type: 'review',
    target_repo: 'example-browser',
    branch: 'main',
    artifact_dir: dir,
    validation_results: {},
    evidence_verdict: 'ok',
  ...
  const result = buildLaneResult({
    ...
    artifactDir: dir,
    learningArtifactPath: path.join(dir, 'recipe-cook-learning.json'),
  });
...
function testMissingCostCannotPromote() {
  const { decisionFromScores } = require('../scripts/lib.cjs');
...
function testRecordValidatorRun() {
  const repoRoot = mkdtempSync(path.join(os.tmpdir(), 'recipe-cook-validator-run-'));
  ...
  fs.mkdirSync(path.join(repoRoot, 'packages', 'skills', 'skills', 'recipe-cook'), {
    recursive: true,
  });
  writeFileSync(
    path.join(repoRoot, 'packages', 'skills', 'skills', 'recipe-cook', 'SKILL.md'),
    'before\n',
  );
  writeJson(path.join(repoRoot, 'recipe-cook', 'assets', 'scenarios.json'), {
    version: 1,
    scenarios: [
      {
        ...
        task_artifact_dir: 'projects/example-browser-farm/tasks/review/41802-0416-1517/artifacts',
      },
      {
        id: 'fix-zero-balance-cta',
        lane: 'fix',
        label: 'Zero-balance CTA bug fix',
        task_artifact_dir: 'projects/example-browser-farm/tasks/fix/41692-0413-2142/artifacts',
      },
      {
        id: 'investigation-probe-output',
        lane: 'investigation',
        label: 'Probe-output investigation lane',
        task_artifact_dir: 'projects/example-browser-farm/tasks/review/41705-0414-1243/artifacts',
      },
    ],
  });
  ...
  const makeScenario = (relDir, semantic, validateExit, cost, lane) => {
    ...
    writeJson(path.join(artifactDir, 'recipe-cook-learning.json'), {
      run_id: `run-${lane}`,
      scenario_type: lane,
      target_repo: 'example-browser',
      branch: 'main',
  ...
  makeScenario(
    'projects/example-browser-farm/tasks/review/41802-0416-1517/artifacts',
    'good',
    0,
    5.5,
    'review',
  ...
  makeScenario(
    'projects/example-browser-farm/tasks/fix/41692-0413-2142/artifacts',
    'good',
    0,
    4.5,
    'fix',
  );
  const probeDir = makeScenario(
    'projects/example-browser-farm/tasks/review/41705-0414-1243/artifacts',
    'good',
    0,
    null,
    'investigation',
  ...
  const script = path.resolve(__dirname, '../scripts/record-validator-run.cjs');
  const res = spawnSync(
    process.execPath,
    [
      script,
      ...
      'run-a',
      '--scenario-config',
      path.join(repoRoot, 'recipe-cook', 'assets', 'scenarios.json'),
      '--touched-file',
      'packages/skills/skills/recipe-cook/SKILL.md',
    ],
  ...
  const runsJsonl = readFileSync(
    path.join(repoRoot, '.omx', 'state', 'recipe-cook', 'runs.jsonl'),
    'utf8',
  )
    .trim()
    .split('\n');
  assert.equal(runsJsonl.length, 3);
  const versionLedger = JSON.parse(
    readFileSync(path.join(repoRoot, '.omx', 'state', 'recipe-cook', 'versions.json'), 'utf8'),
  );
...
function testMissingLaneFails() {
  const repoRoot = mkdtempSync(path.join(os.tmpdir(), 'recipe-cook-validator-missing-lane-'));
  writeJson(path.join(repoRoot, 'recipe-cook', 'assets', 'scenarios.json'), {
    version: 1,
    scenarios: [
...
function testMissingCanonicalScenarioFails() {
  const repoRoot = mkdtempSync(path.join(os.tmpdir(), 'recipe-cook-validator-missing-scenario-'));
  writeJson(path.join(repoRoot, 'recipe-cook', 'assets', 'scenarios.json'), {
    version: 1,
    scenarios: [
      ...
      { id: 'different-fix', lane: 'fix', label: 'fix', task_artifact_dir: 'tmp/fix' },
      {
        ...
        lane: 'investigation',
        label: 'investigation',
        task_artifact_dir: 'tmp/investigation',
      },
    ],
  });
  const script = path.resolve(__dirname, '../scripts/record-validator-run.cjs');
...
function testExtraScenarioFails() {
  const repoRoot = mkdtempSync(path.join(os.tmpdir(), 'recipe-cook-validator-extra-scenario-'));
  writeJson(path.join(repoRoot, 'recipe-cook', 'assets', 'scenarios.json'), {
    version: 1,
    scenarios: [
      {
      ...
      {
        id: 'review-extra-case',
        lane: 'review',
        label: 'extra',
        task_artifact_dir: 'tmp/review-extra',
      },
    ],
  });
  const script = path.resolve(__dirname, '../scripts/record-validator-run.cjs');
...
function testLearningArtifactTouchingFsCookFails() {
  const repoRoot = mkdtempSync(path.join(os.tmpdir(), 'recipe-cook-validator-selfedit-'));
  spawnSync('git', ['init'], { cwd: repoRoot, encoding: 'utf8' });
  spawnSync('git', ['config', 'user.email', 'test@example.com'], {
    cwd: repoRoot,
    encoding: 'utf8',
  ...
  spawnSync('git', ['config', 'user.name', 'Test'], { cwd: repoRoot, encoding: 'utf8' });
  fs.mkdirSync(path.join(repoRoot, 'recipe-cook', 'references'), { recursive: true });
  writeFileSync(path.join(repoRoot, 'recipe-cook', 'SKILL.md'), 'before\n');
  spawnSync('git', ['add', '.'], { cwd: repoRoot, encoding: 'utf8' });
  spawnSync('git', ['commit', '-m', 'init'], { cwd: repoRoot, encoding: 'utf8' });
  writeFileSync(path.join(repoRoot, 'recipe-cook', 'SKILL.md'), 'after\n');
  ...
  writeJson(path.join(artifactDir, 'recipe-cook-learning.json'), {
    ...
    recipe_path: path.join(artifactDir, 'recipe.json'),
    next_delta: 'bad',
    touched_files: ['packages/skills/skills/recipe-cook/SKILL.md'],
  });
  writeJson(path.join(repoRoot, 'recipe-cook', 'assets', 'scenarios.json'), {
    version: 1,
    scenarios: [
      {
        ...
        lane: 'review',
        label: 'review',
        task_artifact_dir: 'projects/example-browser-farm/tasks/review/41802/artifacts',
      },
      {
        id: 'fix-zero-balance-cta',
        lane: 'fix',
  ...
  assert.notEqual(res.status, 0);
  assert.match(res.stderr || res.stdout, /illegally touched recipe-cook/i);
}

function testUnexpectedFsCookDiffFails() {
  const repoRoot = mkdtempSync(path.join(os.tmpdir(), 'recipe-cook-validator-diff-scope-'));
  spawnSync('git', ['init'], { cwd: repoRoot, encoding: 'utf8' });
  spawnSync('git', ['config', 'user.email', 'test@example.com'], {
    cwd: repoRoot,
    encoding: 'utf8',
  ...
  spawnSync('git', ['config', 'user.name', 'Test'], { cwd: repoRoot, encoding: 'utf8' });
  fs.mkdirSync(path.join(repoRoot, '.agents', 'skills', 'recipe-cook', 'references'), {
    recursive: true,
  });
  writeFileSync(
    path.join(repoRoot, '.agents', 'skills', 'recipe-cook', 'references', 'TASK.md'),
    'before\n',
  );
  writeFileSync(path.join(repoRoot, '.agents', 'skills', 'recipe-cook', 'SKILL.md'), 'base\n');
  spawnSync('git', ['add', '.'], { cwd: repoRoot, encoding: 'utf8' });
  spawnSync('git', ['commit', '-m', 'init'], { cwd: repoRoot, encoding: 'utf8' });
  writeFileSync(
    path.join(repoRoot, '.agents', 'skills', 'recipe-cook', 'references', 'TASK.md'),
  ...
  writeFileSync(path.join(repoRoot, '.agents', 'skills', 'recipe-cook', 'SKILL.md'), 'changed\n');
  const artifactDir = path.join(
    repoRoot,
    'projects',
    'example-browser-farm',
  ...
  const res = spawnSync(
    ...
    [
      ...
      path.join(repoRoot, 'recipe-cook', 'assets', 'scenarios.json'),
      '--touched-file',
      '.agents/skills/recipe-cook/references/TASK.md',
    ],
    { cwd: repoRoot, encoding: 'utf8' },
  );
  assert.notEqual(res.status, 0);
  assert.match(res.stderr || res.stdout, /Unexpected recipe-cook diff/i);
}

function testMissingRecipeFails() {
  const repoRoot = mkdtempSync(path.join(os.tmpdir(), 'recipe-cook-validator-missing-recipe-'));
  spawnSync('git', ['init'], { cwd: repoRoot, encoding: 'utf8' });
  spawnSync('git', ['config', 'user.email', 'test@example.com'], {
    cwd: repoRoot,
    encoding: 'utf8',
