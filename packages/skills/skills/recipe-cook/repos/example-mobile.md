...
```bash
rg --files | rg 'recipes?/.*\\.json$|flows?/.*\\.json$|validate-recipe\\.(sh|js|mjs|cjs|ts)|detox|maestro|appium|testIds?|accessibility|agentic-toolkit\\.md'
```
...
- Prefer stable accessibility labels, test IDs, or existing mobile flows when they exist.
