...
- [ ] **7. Draft the recipe** — write `artifacts/recipe.json` and, when supported, `artifacts/recipe-cook.json`. Use documented runner field names for nodes (for example `timeout_ms` / `poll_ms` on `wait_for`), not guessed aliases.
...
- [ ] **10. Run runner dry-run validation** — if the repo provides a runner, run at least a dry-run and record the command + output.
...
- [ ] **15. Final completion gate** — reach the Success outcome only after recipe writing, rewritten TASK.md, validation evidence, learning artifact, and unresolved-target audit are all present.
...
- `artifacts/recipe-cook-learning.json` is missing
...
- the cooking run edits `recipe-cook/SKILL.md`, `recipe-cook/references/TASK.md`, or `recipe-cook/repos/*.md`
