...
const RECIPE_COOK_PATCH_ROOTS = [
  'packages/skills/skills/recipe-cook',
  'recipe-cook',
  '.agents/skills/recipe-cook',
  '.codex/skills/recipe-cook',
  '.claude/skills/recipe-cook',
  '.cursor/skills/recipe-cook',
];

const RECIPE_COOK_PATCH_FILES = new Set([
  'SKILL.md',
  'TASK.md',
  'references/TASK.md',
...
function splitRecipeCookPatchPath(filePath) {
  const normalized = normalizeRepoPath(filePath);
  for (const root of RECIPE_COOK_PATCH_ROOTS) {
    const prefix = `${root}/`;
...
function isAllowedRecipeCookPatchPath(filePath) {
  const parsed = splitRecipeCookPatchPath(filePath);
  return Boolean(
    parsed &&
    (RECIPE_COOK_PATCH_FILES.has(parsed.relativePath) ||
      /^repos\/[^/]+\.md$/.test(parsed.relativePath)),
  );
}
...
function parseArgs(argv = process.argv.slice(2)) {
  const result = {
    ...
    iterationId: '',
    runId: '',
    scenarioConfig: path.resolve(__dirname, '../templates/recipe-cook/scenarios.json'),
    touchedFiles: [],
  };
  for (let i = 0; i < argv.length; i += 1) {
    switch (argv[i]) {
      case '--version-id':
        result.versionId = argv[++i] || '';
        break;
      case '--parent-version-id':
        result.parentVersionId = argv[++i] || '';
        break;
      case '--iteration-id':
        result.iterationId = argv[++i] || '';
        break;
      case '--run-id':
        result.runId = argv[++i] || '';
        break;
      case '--scenario-config':
        ...
        break;
      case '--touched-file':
        result.touchedFiles.push(argv[++i] || '');
        break;
      default:
        break;
    }
...
function createPatchArtifact(
  ...
  const slug = `${versionId || 'version'}-${scenarioIds.join('-') || 'scenario'}`.replace(
    /[^a-zA-Z0-9-_]/g,
    '-',
  );
  ...
  normalizedTouched.forEach((file) => {
    ...
    const diff =
      tracked.status === 0
        ...
        : spawnSync('git', ['diff', '--binary', '--no-index', '--', '/dev/null', file], {
            cwd: baseDir,
            encoding: 'utf8',
          });
...
function main() {
  ...
  const illegalTouched = args.touchedFiles.filter((file) => !isAllowedRecipeCookPatchPath(file));
  assert(
    illegalTouched.length === 0,
    `Validator touched-file outside approved write scope: ${illegalTouched.join(', ')}`,
  );
  ...
  ['review-explore-markets-live', 'fix-zero-balance-cta', 'investigation-probe-output'].forEach(
    (scenarioId) => {
      assert(
  ...
  assert(
    scenarioConfig.scenarios.length === 3,
    'Scenario config must contain exactly the v1 canonical scenario bundle',
  );

  const laneResults = scenarioConfig.scenarios.map((scenario) => {
    const artifactDir = path.resolve(baseDir, scenario.task_artifact_dir);
    const learningArtifactPath = path.join(artifactDir, 'recipe-cook-learning.json');
  ...
  const runsPath = path.join(baseDir, '.omx', 'state', 'recipe-cook', 'runs.jsonl');
  const versionsPath = path.join(baseDir, '.omx', 'state', 'recipe-cook', 'versions.json');
  const versionLedger = loadVersionLedger(versionsPath);
  const previousKept = [...versionLedger.versions]
    .reverse()
    .find((entry) => entry.decision === 'keep');
  const currentRawCost = computeAggregateScore(laneResults, null).raw_inference_cost_usd;
  const previousRawCost =
    previousKept && previousKept.aggregate_score_vector
      ? previousKept.aggregate_score_vector.raw_inference_cost_usd
      : null;
  const costBaseline =
    currentRawCost == null && previousRawCost == null
      ? null
      : Math.max(currentRawCost ?? 0, previousRawCost ?? 0);
  const scoreVector = computeAggregateScore(
    laneResults,
    costBaseline && costBaseline > 0 ? costBaseline : null,
  );
  if (
    previousKept &&
    previousKept.aggregate_score_vector &&
    previousRawCost != null &&
    costBaseline &&
    costBaseline > 0
  ) {
    previousKept.aggregate_score_vector = {
      ...
      balanced_score:
        previousKept.aggregate_score_vector.quality * 0.45 +
        previousKept.aggregate_score_vector.pass_rate * 0.4 +
  ...
  const diffArgs = ['diff', '--name-only', '--', ...RECIPE_COOK_PATCH_ROOTS];
  const currentRecipeCookDiff = spawnSync('git', diffArgs, { cwd: baseDir, encoding: 'utf8' });
  const currentTouchedRecipeCook = currentRecipeCookDiff.stdout
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean);
  const allowedTouched = new Set(args.touchedFiles.filter(Boolean).map(normalizeRepoPath));
  const unexpectedRecipeCookChanges = currentTouchedRecipeCook.filter(
    (file) => !allowedTouched.has(normalizeRepoPath(file)),
  );
  assert(
    unexpectedRecipeCookChanges.length === 0,
    `Unexpected recipe-cook diff outside validator touched-files: ${unexpectedRecipeCookChanges.join(', ')}`,
  );
