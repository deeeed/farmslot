...
function inferRepoName(repoRoot) {
  ...
  if (base.includes('browser')) return 'example-browser';
  if (base.includes('mobile')) return 'example-mobile';
  return base;
}
...
function fetchPrBundle(sourceRef, ghRepo) {
  ...
  assert(
    diff.error?.code !== 'ENOBUFS',
    `gh pr diff exceeded 20MB buffer for PR ${sourceRef}; stream to disk or split the source.`,
  );
...
function resolveJiraSource(sourceRef, jiraBaseUrl) {
  const raw = String(sourceRef || '').trim();
  const urlMatch = raw.match(/https?:\/\/[^/]+\/browse\/([A-Z][A-Z0-9]+-\d+)/i);
  const issueKey = urlMatch ? urlMatch[1].toUpperCase() : raw.toUpperCase();
  const browseUrl = urlMatch
    ? raw
    : `${String(jiraBaseUrl || '').replace(/\/$/, '')}/browse/${issueKey}`;
  return {
    issueKey,
    browseUrl,
  };
...
function fetchJiraBundle(sourceRef, jiraBaseUrl, jiraEmail, jiraToken) {
  assert(jiraBaseUrl, 'Jira source kind requires --jira-base-url or JIRA_BASE_URL');
  const { issueKey, browseUrl } = resolveJiraSource(sourceRef, jiraBaseUrl);
  if (!(jiraEmail && jiraToken)) {
    return [
      `# Source Kind: Jira`,
      `# Source Ref: ${issueKey}`,
      `# Source URL: ${browseUrl}`,
      '',
      '## Jira Issue',
      '',
      'Jira API fetch unavailable from the current shell context.',
      'Use the ticket URL above plus any local notes/branch context as the source of truth for the first iteration.',
      '',
    ].join('\n');
  }
  const url = `${jiraBaseUrl.replace(/\/$/, '')}/rest/api/3/issue/${issueKey}?expand=renderedFields`;
  const res = spawnSync('curl', ['-s', '-u', `${jiraEmail}:${jiraToken}`, url], {
    encoding: 'utf8',
...
function main() {
  ...
  const runId =
    args.runId ||
    `${slugify(`${args.sourceKind}-${args.sourceRef || 'source'}`)}-${new Date().toISOString().replace(/[:.]/g, '-')}`;
  const runDir =
    args.outputDir || path.resolve(args.repoRoot, '.omx', 'artifacts', 'recipe-cook-runs', runId);
  ...
  const taskText = expandTemplate(templateText, {
    WRAPPER: args.wrapper || 'recipe-cook',
    SCENARIO_ID: runId,
