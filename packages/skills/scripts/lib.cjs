...
function resolveInferenceCost(sessionMetrics) {
  if (sessionMetrics && typeof sessionMetrics.costEstimate === 'number')
    return sessionMetrics.costEstimate;
...
const RECIPE_COOK_SELF_EDIT_ROOTS = [
  'recipe-cook/',
...
function isRecipeCookSelfEditPath(filePath) {
  const normalized = normalizeRepoPath(filePath);
  return RECIPE_COOK_SELF_EDIT_ROOTS.some((root) => normalized.startsWith(root));
}
...
function validateLearningArtifact(artifact, expectedArtifactDir) {
  ...
  required.forEach((field) =>
    assert(
      ...
      `Learning artifact missing required field: ${field}`,
    ),
  );
  ...
  assert(
    normalizePath(artifact.artifact_dir) === normalizePath(expectedArtifactDir),
    'Learning artifact artifact_dir does not match scenario artifact dir',
  );
  assert(Array.isArray(artifact.touched_files), 'Learning artifact touched_files must be an array');
  const illegalTouch = artifact.touched_files.find((item) => isRecipeCookSelfEditPath(item));
  assert(!illegalTouch, `Cooking run illegally touched recipe-cook package path: ${illegalTouch}`);
}

function compareScoreVectors(current, previous) {
  const currentBalanced = current.balanced_score;
  const previousBalanced =
    previous && previous.balanced_score != null ? previous.balanced_score : null;
  ...
  if (current.pass_rate < (previous ? previous.pass_rate : -1)) return -1;
  if ((current.cost_efficiency ?? -1) > (previous ? (previous.cost_efficiency ?? -1) : -1))
    return 1;
  if ((current.cost_efficiency ?? -1) < (previous ? (previous.cost_efficiency ?? -1) : -1))
    return -1;
  return 0;
...
function decisionFromScores(scoreVector, laneResults, previousVersion) {
  const floorsMet = laneResults.every(
    (lane) => lane.quality >= 0.8 && lane.pass_rate != null && lane.pass_rate >= 0.8,
  );
  if (!floorsMet) {
  ...
  if (scoreVector.cost_efficiency == null) {
    if (!previousVersion) {
      return {
        decision: 'reject',
        reason: 'Missing cost coverage cannot promote the first candidate to keep.',
      };
    }
    if (
      previousVersion.aggregate_score_vector &&
      previousVersion.aggregate_score_vector.cost_efficiency != null
    ) {
      ...
      if (compare >= 0) {
        return {
          decision: 'reject',
          reason: 'Missing cost coverage cannot replace the current best-known version.',
        };
      }
      return {
        decision: 'reject',
        reason: 'Missing cost coverage loses on quality/pass-rate tie-breaks.',
      };
    }
  }
  ...
  if (compare >= 0) {
    return {
      decision: 'keep',
      reason: 'Candidate improves or matches incumbent under score/tie-break rules.',
    };
  }
...
function computeConvergenceState(versionLedger, runEntries = []) {
  ...
  const plateau =
    ...
    recent.every(
      (entry) =>
        entry.aggregate_score_vector && entry.aggregate_score_vector.balanced_score != null,
    ) &&
    Math.max(...recent.map((entry) => entry.aggregate_score_vector.balanced_score)) -
      Math.min(...recent.map((entry) => entry.aggregate_score_vector.balanced_score)) <=
      0.02;
  ...
  const laneSuccess =
    latestRuns.length > 0 &&
    latestRuns.every(
      (entry) => entry.quality >= 0.9 && entry.pass_rate != null && entry.pass_rate >= 0.9,
    );
  const bestKnownCost =
    versions
      .map(
        (entry) =>
          entry.aggregate_score_vector && entry.aggregate_score_vector.raw_inference_cost_usd,
      )
      .filter((value) => value != null)
      .sort((a, b) => a - b)[0] ?? null;
  const latestCost =
    latest && latest.aggregate_score_vector
      ? latest.aggregate_score_vector.raw_inference_cost_usd
      : null;
  const costWithinBand =
    bestKnownCost != null && latestCost != null && latestCost <= bestKnownCost * 1.1;
  const successBand =
    latest &&
...
function computeAggregateScore(laneResults, baseline) {
  ...
  const costs = laneResults.map((result) =>
    normalizeCostEfficiency(result.inference_cost_usd, baseline),
  );
  const costEfficiency = aggregateAverage(costs);
  const balancedScore =
    costEfficiency == null ? null : quality * 0.45 + passRate * 0.4 + costEfficiency * 0.15;
  return {
    quality,
    pass_rate: passRate,
    cost_efficiency: costEfficiency,
    balanced_score: balancedScore,
    raw_inference_cost_usd: rawCost,
  };
}
...
function buildLaneResult({ lane, scenarioId, artifactDir, learningArtifactPath }) {
  ...
  const liveRunExit =
    meta && typeof meta.validate_exit === 'number' && meta.validate_exit >= 0
      ? meta.validate_exit
      : null;
