...
const { expandTemplate } = require('./lib.cjs');

const DEFAULT_SCENARIO_CONFIG = path.resolve(__dirname, '../templates/recipe-cook/scenarios.json');
const TEMPLATE_PATH = path.resolve(__dirname, '../skills/recipe-cook/references/TASK.md');
const SKILL_PATH = path.resolve(__dirname, '../skills/recipe-cook/SKILL.md');
const OVERLAYS = {
  'example-browser': path.resolve(__dirname, '../skills/recipe-cook/repos/example-browser.md'),
  'example-mobile': path.resolve(__dirname, '../skills/recipe-cook/repos/example-mobile.md'),
};
...
function parseJsonEnvelope(rawText) {
  ...
  try {
    ...
    assert(
      ...
      'Runner output did not contain parseable JSON.',
    );
...
function inferRepoName(scenario, projectRoot) {
  ...
  if (basename.includes('extension')) return 'example-browser';
  if (basename.includes('mobile')) return 'example-mobile';
  return basename;
}
...
function probeCdpPort(cdpPort) {
  ...
  const res = spawnSync(
    'curl',
    ['-s', '--max-time', '2', `http://127.0.0.1:${cdpPort}/json/version`],
    {
      encoding: 'utf8',
    },
  );
...
function resolveSlotContext({ baseDir, slotId, projectName, repoName, repoRoot, cdpPort }) {
  ...
  if (slotId) {
    ...
    candidates = candidates.filter(
      (slot) => slot.repo && path.resolve(slot.repo) === normalizedRepoRoot,
    );
  } else {
    candidates = candidates.filter((slot) => {
      if (projectName && slot.project === projectName) return true;
      if (repoName === 'example-browser' && /example-browser/.test(slot.repo)) return true;
      if (repoName === 'example-mobile' && /example-mobile/.test(slot.repo)) return true;
      return false;
    });
  }
  ...
  const ranked = candidates
    ...
    .sort((left, right) => {
      ...
      if (Boolean(left.slot.cdp_port) !== Boolean(right.slot.cdp_port))
        return left.slot.cdp_port ? -1 : 1;
...
function collectSourceFiles(artifactDir) {
  ...
  fs.readdirSync(artifactDir, { withFileTypes: true }).forEach((entry) => {
    ...
    if (/^(recipe-cook-learning|session-metrics)\.json$/i.test(entry.name)) return;
...
function renderSourceBundle(sourceFiles) {
  ...
  sourceFiles.forEach((filePath) => {
    ...
    const truncated =
      raw.length > MAX_PER_FILE ? `${raw.slice(0, MAX_PER_FILE)}\n...[truncated]` : raw;
    const remaining = MAX_TOTAL - total;
    const value =
      truncated.length > remaining ? `${truncated.slice(0, remaining)}\n...[truncated]` : truncated;
    total += value.length;
...
function buildPrompt({
  ...
  return [
    'You are running the recipe-cook cooking skill as an isolated external runner.',
    ...
    `Read ${skillPath} for the recipe-cook workflow contract.`,
    ...
    `- ${path.join(artifactsDir, 'recipe-cook-learning.json')}`,
    '',
    ...
    '',
    '## Hard requirements',
    '- Follow the recipe-cook workflow order from the task file, skill file, and repo overlay.',
    ...
    '- Do not omit the RECIPE_COOK_VALIDATION_PENDING token.',
...
function runRunnerStream({ prompt, runner, runnerCmd, model, runnerLogPath, promptPath }) {
  const invocation = resolveRunnerInvocation({ runner, runnerCmd, model });
  const logPath = runnerLogPath || path.join(os.tmpdir(), `recipe-cook-runner-${Date.now()}.log`);
  ...
  return new Promise((resolve, reject) => {
    ...
    const emitStatus = (message) => {
      const line = `[recipe-cook] ${new Date().toISOString()} ${message}\n`;
...
function runValidationSteps(steps, validationDir) {
  const results = {};
  steps.forEach((step) => {
    const commandString = step.command
    ...
    writeFile(logPath, [`$ ${commandString}`, '', res.stdout || '', res.stderr || ''].join('\n'));
    results[step.name] = {
...
function renderValidationEvidence(validationResults, cdpPort, repoRoot) {
  ...
  if (
    !validationResults.schema_validation &&
    !validationResults.dry_run &&
    !validationResults.live_run
  ) {
    lines.push(
      `- validation unavailable: no repo-local validator/runner discovered under ${repoRoot || 'unspecified repo root'}`,
    );
  } else if (!validationResults.live_run) {
    if (!cdpPort) {
      lines.push(
        '- live validation unavailable: missing `--cdp-port` for a live runner invocation',
      );
    } else {
...
function injectValidationEvidence(taskMarkdown, evidenceBlock) {
  ...
  if (/## Validation Evidence[\s\S]*?(?=\n## |\s*$)/.test(next)) {
    next = next.replace(
      /## Validation Evidence[\s\S]*?(?=\n## |\s*$)/,
      `## Validation Evidence\n\n${evidenceBlock}\n`,
    );
  } else if (next.includes('RECIPE_COOK_VALIDATION_PENDING')) {
    next = next.replace('RECIPE_COOK_VALIDATION_PENDING', evidenceBlock);
  } else {
    next = `${next.trim()}\n\n## Validation Evidence\n\n${evidenceBlock}\n`;
  }
  next = next.replace(/RECIPE_COOK_VALIDATION_PENDING/g, '').replace(/\n{3,}/g, '\n\n');
...
async function main() {
  ...
  assert(
    ['batch', 'stream'].includes(args.runnerMode),
    `Unsupported --runner-mode: ${args.runnerMode}`,
  );

  const scenarioConfig = readJson(args.scenarioConfig);
  assert(
    Array.isArray(scenarioConfig.scenarios),
    `Invalid scenario config: ${args.scenarioConfig}`,
  );
  ...
  assert(
    fs.existsSync(taskArtifactDir),
    `Scenario artifact dir does not exist: ${taskArtifactDir}`,
  );

  const inferredProjectRoot = path.resolve(
    process.cwd(),
    inferProjectRoot(scenario.task_artifact_dir),
  );
  const projectName = inferProjectName(inferredProjectRoot);
  const repoName = inferRepoName(scenario, inferredProjectRoot);
  const repoLocalContext =
    !args.repoRoot && !args.slotId
  ...
  const projectRoot =
    slotContext.repoRoot ||
    (repoLocalContext ? repoLocalContext.repoRoot : '') ||
    args.repoRoot ||
    inferredProjectRoot;
  const effectiveCdpPort =
    slotContext.cdpPort || (repoLocalContext ? repoLocalContext.cdpPort : '') || args.cdpPort;
  const overlayPath = OVERLAYS[repoName];
  assert(
    overlayPath && fs.existsSync(overlayPath),
    `Missing recipe-cook overlay for repo: ${repoName}`,
  );

  const runId =
    args.runId || `${slugify(scenario.id)}-${new Date().toISOString().replace(/[:.]/g, '-')}`;
  const runDir =
    args.outputDir ||
    path.resolve(process.cwd(), '.omx', 'artifacts', 'recipe-cook-runs', scenario.id, runId);
  const artifactsDir = path.join(runDir, 'artifacts');
  const validationDir = path.join(artifactsDir, 'validation');
  const runnerLogPath =
    args.runnerLog || (args.runnerMode === 'stream' ? path.join(runDir, 'runner.log') : '');
  fs.mkdirSync(artifactsDir, { recursive: true });
  fs.mkdirSync(validationDir, { recursive: true });

  assert(fs.existsSync(SKILL_PATH), `Missing recipe-cook skill: ${SKILL_PATH}`);
  ...
  const taskTemplateVars = {
    ...
    SOURCE_BUNDLE_FILE: sourceBundlePath,
    WRAPPER: 'recipe-cook',
    VALIDATION_MODE: effectiveCdpPort ? 'live' : 'batch-isolated',
  };
  ...
  writeJson(path.join(runDir, 'run-meta.json'), {
    ...
    cdp_port: effectiveCdpPort || null,
    source_artifact_dir: taskArtifactDir,
    run_dir: runDir,
  ...
  assert(
    ...
    `Runner exited with status ${runnerResult.status}. See ${path.join(runDir, 'runner-output.txt')}`,
  );

  const envelope = parseJsonEnvelope(rawOutput);
  assert(
    typeof envelope.task_markdown === 'string' && envelope.task_markdown.trim(),
    'Runner output missing task_markdown',
  );
  assert(
    envelope.recipe_json &&
      typeof envelope.recipe_json === 'object' &&
      !Array.isArray(envelope.recipe_json),
    'Runner output missing recipe_json object',
  );
  assert(
    envelope.recipe_cook_json == null ||
      (typeof envelope.recipe_cook_json === 'object' && !Array.isArray(envelope.recipe_cook_json)),
    'recipe_cook_json must be an object or null',
  );
  assert(
  ...
  const recipeCookPath = envelope.recipe_cook_json
    ? path.join(artifactsDir, 'recipe-cook.json')
    : null;
  ...
  writeJson(path.join(artifactsDir, 'meta.json'), {
    ...
    validate_exit: validationResults.live_run
      ? validationResults.live_run.exit_code
      : validationResults.dry_run
        ? validationResults.dry_run.exit_code
        : -1,
    outcome:
      terminalState.outcome === 'failure'
        ? 'failed'
  ...
  writeJson(path.join(artifactsDir, 'recipe-cook-learning.json'), {
    run_id: runId,
