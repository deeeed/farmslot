...
- `resolveTaskPaths` accepts a slot `taskFile` that already includes `temp/tasks/` and `TASK.md`.
...
- feat(runtime): accept `runtime_capabilities.host_pressure_admission` — a required `mode` (`off` |
  ...
  non-positive multiplier, and a percent outside 0..100 are all rejected rather than defaulted, and
  an unrecognized mode is dropped in normalization too, so a typo can never fall through as an
  enforcing one. Omitting the block normalizes to nothing and the gateway applies `off`, which is
  what every project that has never declared it gets.
- feat(runtime): accept optional `scope` on a runtime capability cost resource (`slot` | `machine` | `fleet`) and normalize an omitted one to `slot`, so every catalog entry carries an explicit scope and nothing downstream re-applies the default. An unknown value is rejected rather than defaulted: a misspelled scope would silently narrow a fleet-wide device back to its own slot, which is the failure scopes exist to prevent. Every existing project stays valid and behaves as it did.
- test(hooks): lock the precedence a re-targeted capability lease depends on — `extraVars` beats the slot's own resource fields for `{{simulator}}`, `{{avd}}`, `{{adb_serial}}` and `{{platform}}`, and fills a device placeholder the slot does not configure at all. The behaviour already existed; nothing in it changed.
- feat(posture): accept optional `affected_resources` on a runtime capability provider — the slot resources it touches, each with an `ownership` (`capability` or `slot-lifecycle`) and a `release_effect` (`stop` or `retain`). A `resource_id` that names no resource, an unknown enum value, a duplicate resource, and a non-array are all rejected; the resource lookup checks own properties, so an inherited key like `constructor` no longer passes as a resource id. A claim must also match the hooks its resource has: `stop` requires `boot` and `shutdown` because the park stops it and the restore boots it back, and `retain` requires a `health` hook because restore proves it stayed up instead, which a `watch` cannot carry since polling reports a resource without a health hook as `unknown`. Omitting the field normalizes to nothing, so machine parking keeps deriving the claim from the action refs; an empty array survives normalization as the distinct declaration that the provider touches no watched resource.
