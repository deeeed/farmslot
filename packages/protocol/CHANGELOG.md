...
- Independent-review `retryCount` is the max of the persisted field and spent attempts so a wiped `0` cannot hide a spent loop. Exhaustion (and the dangerous bypass) stays visible when the package HEAD has moved since the last extra-review. Pipeline progress labels include independent-review (`human-gate`) while an extra-review checklist is running.
