...
- fix(run): add `RunMonitorState.budgetNudgeAttempts`, `RunMonitorBudgetUsageState.skippedOversizedRecords`, `RunMonitorBudgetUsageState.lastCumulative` (the reference reading for runners that restate session totals), `RunMonitorBudgetUsageState.discardNextRecord`, and `RunMonitorState.budgetFirstDeferredAt`. `baselineTurns`/`baselineTotalTokens` remain only as legacy fields migrated on restore. All are restored on monitor recovery.
