...
- feat(run): add `run.sessionCommand` (`RunSessionCommandParams`/`RunSessionCommandResult`) for reopening an agent context's runner session, and `AgentContext.runnerSessionCapturedAt` recording when that session identity was bound.
