...
- fix(slots): a detached HEAD counts as an idle slot branch. `isSlotIdleBranch` and `isSlotRefreshStaleBranch` treated the literal `HEAD` that `git rev-parse --abbrev-ref` reports for a detached checkout as leftover work, so a slot whose branch was deliberately taken out of the tree carried the stale-branch dispatch penalty and forced an operator pick. A detached HEAD holds no branch ref for a prepare to clobber.
