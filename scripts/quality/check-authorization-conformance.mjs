...
// These methods expose only the caller's native workspace, not global farm state.
// Target ownership remains mandatory at runtime even for an enrolled principal.
const nativeOwnerMethods = new Set([
  'gateway.ping',
  'native.profile.list',
  'native.profile.add',
  'native.profile.status',
  'native.profile.remove',
  'native.session.catalog',
  'native.session.list',
  'native.session.create',
  'native.session.ensure',
  'native.session.read',
  'native.session.send',
...
for (const method of nativeOwnerMethods) {
  if (!registrySet.has(method) || !routed.has(method))
    problems.push(`Native owner method is not routed: ${method}`);
}
...
function renderMarkdown(classifications) {
  const lines = [
    ...
    'The table is normative; unlisted or unproven methods are admin-only. Native-owner methods require an enrolled principal and a separately authorized execution target; they do not grant farm roles.',
    '',
