...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--runner') args.runner = argv[++i];
      ...
      console.log(
        'usage: runner-validation/run.mjs [--runner claude|codex|cursor|grok|both|hooks|pane-only|all] [--scenario hook-smoke|pane-smoke|interaction-smoke|prompt-accepted|turn-boundary|busy-composer|mode-switch|session-attribution-smoke|token-usage-smoke|all] [--out-dir path] [--timeout-ms 300000] [--keep-session]',
      );
      process.exit(0);
    } else {
