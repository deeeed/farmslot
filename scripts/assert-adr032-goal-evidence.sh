...
if [[ -f "$REPLAY_DIR/pr81-premerge-capture.json" && -f "$REPLAY_DIR/pr81-postmerge.json" ]]; then
  ...
  echo "  Historical PRs #81–#86 lack pre-merge GH APPROVED; run replay PR per strategy.md"
fi
