...
Default `farmslot-farm` prepare (`sandbox` / `attach`) boots **gateway + Command Center UI only**.
...
2. Boot if not running (prepare does **not** auto-boot):
   ...
   open -a Simulator
   ```
   Or from Command Center: resource `ios-sim` → **boot** (uses `projects/farmslot-companion/project.json` hooks when dispatched as that project).
...
- Do not assume `projects/farmslot-companion` pool slots exist on every machine; when absent,
...
- Companion recipes: `projects/farmslot-companion` `recipe_run` hook when slot is configured for that project.